...
def train(image_name, resource_folder, processor):
    docker_cmd = 'docker' if processor == 'cpu' else 'nvidia-docker'
...
class Container(object):
    def __init__(self, image, processor, startup_delay=1):
        ...
        self.docker_cmd = 'docker' if processor == 'cpu' else 'nvidia-docker'
    ...
    def execute_command(self, cmd):
        ...
        docker_cmd = [self.docker_cmd, 'exec', '-t', self.name]
...
class HostingContainer(Container):
    def __init__(self, image, opt_ml, script_name, processor, region='us-west-2',
                 requirements_file=None, startup_delay=5):
    ...
    def __enter__(self):
        cmd = [self.docker_cmd,
               ...
               '-e', 'SAGEMAKER_REGION={}'.format(self.region),
